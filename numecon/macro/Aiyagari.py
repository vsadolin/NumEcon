...
class AiyagariModel:
    ...
    def setup_misc(self):
        ...
        trans_p_z = self.trans_p_z.copy()
        for _ in range(1000):
            trans_p_z = trans_p_z@self.trans_p_z
        z_diag = np.diag(trans_p_z)
    ...
    def find_stationary_equilibrium(self,print_results=True):
        ...
        # narrow range to look in
        self.R_low = 0.9*self.R_ss + 0.1*self.R_low
        self.R_high = 0.9*self.R_ss + 0.1*self.R_high
